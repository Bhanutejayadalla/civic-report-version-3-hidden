...
class CivicReportSupabaseDB {
    constructor() {
        ...
        const SUPABASE_URL = 'https://agieoegjcibzvpaashbr.supabase.co';
        const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFnaWVvZWdqY2lienZwYWFzaGJyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg4Nzc2NjgsImV4cCI6MjA3NDQ1MzY2OH0.1rKvI0jPDtHkkBUsE-_O8Fzai_tQkxot0nIoXklPKtM';
    ...
    async initializeDefaultData() {
        ...
        if (categories.length === 0) {
            const defaultCategories = [
                { name: 'Potholes', department_mapping: 'Public Works' },
                { name: 'Street Lights', department_mapping: 'Electric Department' },
                { name: 'Trash Collection', department_mapping: 'Sanitation' },
                { name: 'Water Issues', department_mapping: 'Water Department' },
                { name: 'Traffic Signs', department_mapping: 'Traffic Management' },
                { name: 'Graffiti', department_mapping: 'Public Works' },
                { name: 'Parks & Recreation', department_mapping: 'Parks Department' },
                { name: 'Other', department_mapping: 'General Administration' }
            ];
    ...
    async deleteReport(id) {
        if (this.currentUserRole !== 'admin') return this.showToast('Admins only', 'error');
    ...
    async deleteUser(id) {
        if (this.currentUserRole !== 'admin') return this.showToast('Admins only', 'error');
        if (id === this.currentUser.id) return this.showToast('Cannot delete self', 'error');
    ...
    async handleAdminLogin(e) {
        ...
        try {
            ...
            if (user) {
                ...
                this.currentUserRole = user.role;                       // store role
